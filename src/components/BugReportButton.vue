<template>
  <div>
    <button @click="openBugReportModal" class="bug-report-btn">
      <!-- Bug SVG Icon -->
      <svg class="bug-icon" fill="#000000" width="15px" height="15px" version="1.1" id="Capa_1"
    ...
    <!-- BugReportModal component directly within the button -->
    <BugReportModal ref="modalRef" />
  </div>
</template>
...
export default defineComponent({
  ...
  setup() {
    const modalRef = ref<InstanceType<typeof BugReportModal> | null>(null);

    const openBugReportModal = () => {
      modalRef.value?.openModal(); // Use optional chaining in case modalRef is null
    };
